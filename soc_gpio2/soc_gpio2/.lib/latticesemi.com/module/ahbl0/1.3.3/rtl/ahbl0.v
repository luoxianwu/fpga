...
module ahbl0_ipgen_lscc_ahbl_arbiter #(parameter TOTAL_MASTER_CNT = 8, 
        parameter FIRST_PRIO = ((TOTAL_MASTER_CNT > 2) ? {{(TOTAL_MASTER_CNT - 1){1'b0}},
                1'b1} : 1'b0), 
        ...
        parameter [0:0] EARLY_BURST_TERM_PROTECT = 1'b0) (
    // -----------------------------------------------------------------------------
    // Module Parameters
    // -----------------------------------------------------------------------------
    // KD
    ...
    output reg ahbl_busy_o,  // KD
    output reg [(TOTAL_MASTER_CNT - 1):0] ahbl_done_o,  // KD
    ...
    generate
        if ((ARBITER_SCHEME == 3'b0)) 
            begin : round_robin
                ...
                if (EARLY_BURST_TERM_PROTECT) 
                    begin : gnt_and_done_ctrl_with_burst_term_protect
                        always
                            @(*)
                            begin
                                ahbl_gnt_o = gnt_w ;
                    ...
                    begin : gnt_and_done_ctrl_without_burst_term_protect
                        always
                            @(*)
                            begin
                                ahbl_gnt_o = gnt_w ;
                ...
                // If EARLY_BURST_TERM_PROTECT = 1, the "FIRST_PRIO" parameter will only be passed
                // into "lscc_ahbl_fair_arb".
                if (EARLY_BURST_TERM_PROTECT) 
                    begin : fair_arb_with_early_burst_term_protect
                        ahbl0_ipgen_lscc_ahbl_fair_arb #(.NUM_REQ(TOTAL_MASTER_CNT),
                                .FIRST_PRIO(FIRST_PRIO),
                                .ONEHOTPRIO(1'b1),
                                .EARLY_BURST_TERM_PROTECT(EARLY_BURST_TERM_PROTECT)) u_fair_arb (// number of requestor
                                // KD
                                .clk(ahbl_hclk_i), 
                    ...
                    begin : fair_arb_without_early_burst_term_protect
                        ahbl0_ipgen_lscc_ahbl_fair_arb #(.NUM_REQ(TOTAL_MASTER_CNT),
                                .ONEHOTPRIO(1'b1),
                                .EARLY_BURST_TERM_PROTECT(EARLY_BURST_TERM_PROTECT)) u_fair_arb (// number of requestor
                                .clk(ahbl_hclk_i), 
...
module ahbl0_ipgen_lscc_ahbl_arbmux #(parameter TOTAL_MASTER_CNT = 8, 
    ...
    output wire [(TOTAL_MASTER_CNT - 1):0] ahbl_grant_active_slv_o, 
    input wire [(TOTAL_MASTER_CNT - 1):0] ahbl_grant_active_slv_i, 
    // ----------------------------------------
    ...
    // Local Parameters
    // ------------------------------------------------------------------------------
    localparam FIRST_PRIO = ((TOTAL_MASTER_CNT > 2) ? {{(TOTAL_MASTER_CNT - 1){1'b0}},
                1'b1} : 1'b0) ; // KD
    localparam FIRST_PRIO_MASK = ((FIRST_PRIO > 0) ? FIRST_PRIO : 1) ; // KD
    localparam MSKIDLE = 4'b0001 ; 
    localparam MSKWTMAS = 4'b0010 ; // wait for both new mask and ending mas ready/done
    localparam MSKWTSMAS = 4'b0100 ; // wait for new mas ready
    localparam MSKWTEMAS = 4'b1000 ; // wait for ending mas ready/done
    ...
    reg [3:0] cs_masksm ; 
    reg [3:0] ns_masksm ; 
    reg [(TOTAL_MASTER_CNT - 1):0] smask ; 
    reg [(TOTAL_MASTER_CNT - 1):0] emask ; 
    wire arc_MSKIDLE_MSKWTMAS ; 
    wire arc_MSKIDLE_MSKWTSMAS ; 
    wire arc_MSKIDLE_MSKWTEMAS ; 
    wire arc_MSKWTMAS_MSKIDLE ; 
    wire arc_MSKWTMAS_MSKWTSMAS ; 
    wire arc_MSKWTMAS_MSKWTEMAS ; 
    wire arc_MSKWTSMAS_MSKIDLE ; 
    wire arc_MSKWTSMAS_MSKWTMAS ; 
    wire arc_MSKWTEMAS_MSKIDLE ; 
    wire resume_from_mskwtmas ; 
    wire resume_from_mskwtsmas ; 
    wire resume_from_mskwtemas ; 
    wire resume_from_mskwtmas_qual ; 
    wire resume_from_mskwtsmas_qual ; 
    wire resume_from_mskwtemas_qual ; 
    wire resume_mstr_o ; 
    wire enable_mstr_o ; 
    wire grant_event ; 
    wire pregrant_event ; 
    ...
    wire busy ; // KD
    wire [(TOTAL_MASTER_CNT - 1):0] done ; // KD
    ...
    wire cnt_en_w ; 
    wire arb_ack_w ; 
    // This wire is created to switch the signal behavior of the "ahbl_hsel_mstr_o" output
    // based on the "EARLY_BURST_TERM_PROTECT" parameter.
    wire ahbl_hsel_mstr_w ; 
    ...
    generate
        if (EARLY_BURST_TERM_PROTECT) 
            begin : combi_logic_with_burst_term_protect
                assign ahbl_hreadyout_w = ({TOTAL_MASTER_CNT{(ahbl_hreadyout_mstr_i | resume_mstr_o)}} & grant) ; // KD add resume_mstr_o 
                assign ahbl_grant_active_slv_o = ((cs_sm != ST_IDLE) ? (ahbl_grant_slv_o & (~done)) : {TOTAL_MASTER_CNT{1'b0}}) ; // KD
                assign ahbl_hsel_mstr_w = (enable_mstr_o ? (|(ahbl_hsel_slv_i & grant)) : 1'b0) ; // KD
                assign grant_event = ((|req_in) & (((cs_sm == ST_IDLE) & (~busy)) | (grant_nxt != grant))) ; 
                assign pregrant_event = (((cs_sm == ST_IDLE) & (grant == req_in)) & (|(ahbl_grant_active_slv_i & grant))) ; // new request that same as parked/default grant (may not work for fixed prio arb)
                assign s_ready = (|(ahbl_hready_slv_i & grant_nxt)) ; 
                assign e_ready = ((|(ahbl_hready_slv_i & grant)) | (cs_sm == ST_IDLE)) ; 
                assign resume_mstr_o = ((resume_from_mskwtmas_qual | resume_from_mskwtsmas_qual) | resume_from_mskwtemas_qual) ; 
                assign enable_mstr_o = (((cs_masksm == MSKIDLE) & (~pregrant_event)) | resume_mstr_o) ; 
                assign resume_from_mskwtmas_qual = ((cs_masksm == MSKWTMAS) & resume_from_mskwtmas) ; 
                assign resume_from_mskwtsmas_qual = ((cs_masksm == MSKWTSMAS) & resume_from_mskwtsmas) ; 
                assign resume_from_mskwtemas_qual = ((cs_masksm == MSKWTEMAS) & resume_from_mskwtemas) ; 
                assign arc_MSKIDLE_MSKWTMAS = ((grant_event & (~e_ready)) & (~s_ready)) ; // on going not done, new not ready
                assign arc_MSKIDLE_MSKWTSMAS = ((grant_event & e_ready) & (~s_ready)) ; // on going done, new not ready
                assign arc_MSKIDLE_MSKWTEMAS = ((grant_event & (~e_ready)) & s_ready) ; // on going not done, new ready
                assign arc_MSKWTMAS_MSKIDLE = (resume_from_mskwtmas & (~grant_event)) ; // no nested new gnt that need to full WTSMAS
                // on going done
                assign resume_from_mskwtmas = ((|(ahbl_hready_slv_i & emask)) & (|(ahbl_hready_slv_i & smask))) ; // new not ready
                // on going done
                // new not ready
                assign arc_MSKWTMAS_MSKWTSMAS = (((|(ahbl_hready_slv_i & emask)) & (~(|(ahbl_hready_slv_i & smask)))) & (~grant_event)) ; // no nested new gnt that need to full WTSMAS
                // on going not done
                // new ready
                assign arc_MSKWTMAS_MSKWTEMAS = (((~(|(ahbl_hready_slv_i & emask))) & (|(ahbl_hready_slv_i & smask))) & (~grant_event)) ; // no nested new gnt that need to full WTSMAS
                // back to back grant case
                assign arc_MSKWTSMAS_MSKIDLE = (resume_from_mskwtsmas & (~grant_event)) ; // no new gnt, or new gnt no throttle
                assign arc_MSKWTSMAS_MSKWTMAS = (resume_from_mskwtsmas & grant_event) ; 
                assign resume_from_mskwtsmas = (|(ahbl_hready_slv_i & smask)) ; 
                assign arc_MSKWTEMAS_MSKIDLE = resume_from_mskwtemas ; 
                assign resume_from_mskwtemas = (|(ahbl_hready_slv_i & emask)) ; 
            end
        else
            begin : combi_logic_without_burst_term_protect
                assign ahbl_hreadyout_w = ({TOTAL_MASTER_CNT{ahbl_hreadyout_mstr_i}} & grant) ; 
                assign ahbl_grant_active_slv_o = {TOTAL_MASTER_CNT{1'b0}} ; 
                assign ahbl_hsel_mstr_w = (|(ahbl_hsel_slv_i & grant)) ; 
            end
    endgenerate
    // ------------------------------------------------------------------------------
    // Generate Blocks
            ...
            begin : is_on_outreg_mstr_off
                always
                    ...
                    begin
                        ...
                        ahbl_hsel_mstr_o = ahbl_hsel_mstr_w ;
            ...
            begin : masksm_fsm
                always
                    ...
                    begin
                        ...
                        MSKWTMAS : 
                            begin
                                ns_masksm = (arc_MSKWTMAS_MSKIDLE ? MSKIDLE : (arc_MSKWTMAS_MSKWTSMAS ? MSKWTSMAS : (arc_MSKWTMAS_MSKWTEMAS ? MSKWTEMAS : MSKWTMAS))) ;
                            end
                        MSKWTSMAS : 
                            begin
                                ns_masksm = (arc_MSKWTSMAS_MSKIDLE ? MSKIDLE : (arc_MSKWTSMAS_MSKWTMAS ? MSKWTMAS : MSKWTSMAS)) ;
                            end
                        MSKWTEMAS : 
                            begin
                                ns_masksm = (arc_MSKWTEMAS_MSKIDLE ? MSKIDLE : MSKWTEMAS) ;
                            end
                        default : 
                            begin
                                ns_masksm = (arc_MSKIDLE_MSKWTMAS ? MSKWTMAS : (arc_MSKIDLE_MSKWTSMAS ? MSKWTSMAS : (arc_MSKIDLE_MSKWTEMAS ? MSKWTEMAS : MSKIDLE))) ;
                            end
                        endcase 
                    end
            end
        // If EARLY_BURST_TERM_PROTECT = 1, there will be an additional "arb_en = 1'b1" under
        // the "ST_ACTIVE" state.
        if (EARLY_BURST_TERM_PROTECT) 
            begin : main_fsm_with_burst_term_protect
                always
                    @(*)
                    begin
                        ns_sm = cs_sm ;
                                ...
                                if ((((((ahbl_htrans == IDLE) || (ahbl_htrans == NSEQ)) && ahbl_hreadyout_mstr_i) && (~ahbl_hresp_mstr_i)) && (~start_of_burst_w))) 
                                    begin
                                        if ((|req_in)) 
                                            ...
                                            begin
                                                arb_en = 1'b1 ;// KD pulse this to generate done 
                                                ns_sm = ST_IDLE ;
                                            end
                                    end
                                else
            ...
            begin : main_fsm_without_burst_term_protect
                always
                    @(*)
                    begin
                        ns_sm = cs_sm ;
                    ...
                    begin
                        if ((~ahbl_hresetn_i)) 
                            begin
                                ...
                                // KD.
                                cs_masksm <=  MSKIDLE ;
                                smask <=  FIRST_PRIO_MASK[(TOTAL_MASTER_CNT - 1):0] ;
                                emask <=  FIRST_PRIO_MASK[(TOTAL_MASTER_CNT - 1):0] ;
                            end
                        else
                            begin
                                cs_sm <=  ns_sm ;
    ...
    generate
        // If EARLY_BURST_TERM_PROTECT = 1, the "FIRST_PRIO" parameter will be passed, and the
        // "ahbl_done_o" and "ahbl_busy_o" ports will be connected.
        if (EARLY_BURST_TERM_PROTECT) 
            begin : arbiter_with_burst_term_protect
                ahbl0_ipgen_lscc_ahbl_arbiter #(.TOTAL_MASTER_CNT(TOTAL_MASTER_CNT),
                        ...
                        .EARLY_BURST_TERM_PROTECT(EARLY_BURST_TERM_PROTECT)) u_lscc_ahbl_arbiter (//AUTOINSTPARAM*/
                        // Parameters
                        // KD	   
                        /*/UTOINST*/ // Outputs
                        .ahbl_gnt_o(grant), 
            ...
            begin : arbiter_without_burst_term_protect
                ahbl0_ipgen_lscc_ahbl_arbiter #(.TOTAL_MASTER_CNT(TOTAL_MASTER_CNT),
                        .M_PRIO_IDX(M_PRIO_IDX),
                        .DATA_WIDTH(DATA_WIDTH),
                        .ARBITER_SCHEME(ARBITER_SCHEME),
...
module ahbl0_ipgen_lscc_ahbl_crossbar #(parameter F = 8, 
    ...
    wire [((M * N) - 1):0] ahbl_grant_active_slv ; 
    wire [((M * N) - 1):0] ahbl_grant_active_slv_in_master_slave_array_w ; 
    wire [((M * N) - 1):0] ahbl_grant_active_slv_ored_back_to_each_arbmux ; 
    // ------------------------------------------------------------------------------
    // Assign Statements
    // ------------------------------------------------------------------------------
    genvar i, 
    ...
    generate
        if (EARLY_BURST_TERM_PROTECT) 
            begin : rearrange_ahbl_grant_active_slv
                for (j = 0;(j < N);j = (j + 1))
                begin : genblk1
                    for (i = 0;(i < M);i = (i + 1))
                    begin : genblk1
                        // "ahbl_grant_active_slv_in_master_slave_array_w" will be arranged in the following
                        // format if there are 10 masters and 7 slaves:
                        // Master 0, slave 0's bit 0 -> 0
                        // Master 0, slave 1's bit 0 -> 1
                        // Master 0, slave 2's bit 0 -> 2
                        // Master 0, slave 3's bit 0 -> 3
                        //              .
                        //              .
                        //              .
                        // Master 1, slave 0's bit 0 -> 7
                        // Master 1, slave 1's bit 0 -> 8
                        // Master 1, slave 2's bit 0 -> 9
                        assign ahbl_grant_active_slv_in_master_slave_array_w[((i * N) + j)] = ahbl_grant_active_slv[((j * M) + i)] ; 
                    end
                end
            end
        for (i = 0;(i < M);i = (i + 1))
        begin : intercon
            ...
            begin : genblk1
                if (CONNECT_EN[((i * 32) + j)]) 
                    begin : genblk1
                        if (EARLY_BURST_TERM_PROTECT) 
                            begin : return_modified_grant_to_each_arbmux
                                // "ahbl_grant_active_slv_ored_back_to_each_arbmux" will be arranged in the
                                // following format:
                                // Bit[9:0]     for arbmux[0] (with grant from arbmux[0] cleared to zero)
                                // Bit[19:10]   for arbmux[1] (with grant from arbmux[1] cleared to zero)
                                // Bit[29:20]   for arbmux[2] (with grant from arbmux[2] cleared to zero)
                                //            .
                                //            .
                                //            .
                                // Bit[69:60]   for arbmux[6] (with grant from arbmux[6] cleared to zero)
                                assign ahbl_grant_active_slv_ored_back_to_each_arbmux[((j * M) + i)] = (|(ahbl_grant_active_slv_in_master_slave_array_w[(i * N) +: N] & mask_to_clear_one_bit(j))) ; 
                            end
    ...
    generate
        ...
        begin : arbiter_mux
            ...
            // -----------------------------
            // The following two signals will be included/removed based on the value of
            // "EARLY_BURST_TERM_PROTECT":
            //   • ahbl_grant_active_slv_i
            //   • ahbl_grant_active_slv_o
            if (EARLY_BURST_TERM_PROTECT) 
            // for (i = 0; i < N; i = i + 1)
                begin : arbmux_with_burst_term_protect
                    ahbl0_ipgen_lscc_ahbl_arbmux #(.TOTAL_MASTER_CNT(TOTAL_MASTER_CNT),
                            ...
                            .DATA_WIDTH(DATA_WIDTH),
                            .OUTREG_EN(OUTREG_EN),
                            .EARLY_BURST_TERM_PROTECT(EARLY_BURST_TERM_PROTECT),
                            ...
                            .ahbl_hreadyout_slv_o(ahbl_arb_s2m_hreadyout[(M * i) +: M]), 
                                ...
                                .ahbl_grant_active_slv_i(ahbl_grant_active_slv_ored_back_to_each_arbmux[(i * M) +: M]), 
                                .ahbl_grant_active_slv_o(ahbl_grant_active_slv[(M * i) +: M]), 
                                // Inputs
                            .ahbl_hclk_i(ahbl_hclk_i), 
                                .ahbl_hresetn_i(ahbl_hresetn_i), 
                ...
                begin : arbmux_without_burst_term_protect
                    ahbl0_ipgen_lscc_ahbl_arbmux #(.TOTAL_MASTER_CNT(TOTAL_MASTER_CNT),
                            .M_ADDR_WIDTH(M_ADDR_WIDTH),
                            .M_PRIO_WIDTH(M_PRIO_WIDTH),
                            .M_PRIO_IDX(M_PRIO_IDX[((M * M_PRIO_WIDTH) * i) +: (M * M_PRIO_WIDTH)]),
                            ...
                            .ahbl_hreadyout_slv_o(ahbl_arb_s2m_hreadyout[(M * i) +: M]), 
                                ...
                                .ahbl_wait_slv_o(ahbl_wait_s2m_w[(M * i) +: M]), 
                                .ahbl_addr_stall_o(ahbl_addr_stall_w[(M * i) +: M]), 
                                .ahbl_grant_active_slv_i({TOTAL_MASTER_CNT{1'b0}}), 
                                .ahbl_grant_active_slv_o(), 
                                // Inputs
                            .ahbl_hclk_i(ahbl_hclk_i), 
                                .ahbl_hresetn_i(ahbl_hresetn_i), 
    ...
    // This function returns a mask, by performing and "AND" operation between a variable and
    // the returned mask will clear a bit, in which the bit index is specified through the
    // "index" function parameter.
    // NOTE: "TOTAL_SLAVE_CNT" is used to define the size of the returned mask as it is only
    // used to mask with variables of the same size. You may change this definition factor if
    // it is used with other variable sizes in future revisions.
    function [(TOTAL_SLAVE_CNT - 1):0] mask_to_clear_one_bit( 
        input integer index )  ; 
        mask_to_clear_one_bit = ({TOTAL_SLAVE_CNT{1'b1}} & (~(1'b1 << index))) ;
    endfunction
endmodule
...
module ahbl0_ipgen_lscc_ahbl_fair_arb #(parameter NUM_REQ = 4, 
        parameter FIRST_PRIO = ((NUM_REQ > 2) ? {{(NUM_REQ - 1){1'b0}},
                1'b1} : 1'b0), 
    ...
    generate
        if ((NUM_REQ > 2)) 
            ...
            begin : gen_arb_2_req
                ...
                if (EARLY_BURST_TERM_PROTECT) 
                    begin : gnt_nxt_ctrl
                        assign gnt_nxt[0] = (~gnt_nxt_w) ; // KD
                        assign gnt_nxt[1] = gnt_nxt_w ; // KD
                    end
                //--------------------------------------------
                //-- New grant --
                //--------------------------------------------
...
module ahbl0_ipgen_lscc_ahbl_interconnect #(parameter F = 8, 
    ...
    //parameter                                     K                  = 32,
    // 05/03 prev: 0
    // Setting this parameter to 1 will prevent the IP from initiating early burst
    // terminations when the transactions to the external slaves are back-to-back and between
    // any of those transactions, access changes from one external slave to another. This
    // parameter can be left unset if the external slaves connected are able to handle burst
    // terminations initiated by the IP itself.
    // 0 or 1.
    //--- Slave 0 -------------------------------------------------------------------
    //--- Slave 1 -------------------------------------------------------------------
    //--- Slave 2 -------------------------------------------------------------------
